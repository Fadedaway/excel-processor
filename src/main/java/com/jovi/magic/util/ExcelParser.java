...
public class ExcelParser {
     ...
     * @return 表格转换器
     */
    public ExcelParser parse(String fileName, int columsLength)
            throws InvalidFormatException, IOException, ParseException {
        return parse(fileName, 1, columsLength);
    }
     ...
     * @return 表格转换器
     */
    public synchronized ExcelParser parse(String fileName, int sheetId, int columsLength)
            throws InvalidFormatException, IOException, ParseException {
        // 每次转换前都清空数据
        datas.clear();
        // 打开表格文件输入流
